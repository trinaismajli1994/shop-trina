...
<form role="search" method="get" class="woocommerce-product-search" action="<?php echo esc_url( home_url( '/' ) ); ?>">
	...
	<input type="search" id="woocommerce-product-search-field-<?php echo isset( $botiga_unique_id ) ? esc_attr( $botiga_unique_id ) : 0; ?>" class="search-field" placeholder="<?php echo esc_attr__( 'Kërko produktet&hellip;', 'botiga' ); ?>" value="<?php echo get_search_query(); ?>" name="s" />
	<button type="submit" class="search-submit" value="<?php echo esc_attr_x( 'Search', 'submit button', 'botiga' ); ?>"><i class="ws-svg-icon"><?php botiga_get_svg_icon( 'icon-search', true ); ?></i></button>
