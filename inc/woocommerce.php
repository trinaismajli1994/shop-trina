...
if ( ! function_exists( 'botiga_woocommerce_cart_link' ) ) {
	...
	function botiga_woocommerce_cart_link() {
		...
		$link = '<a class="cart-contents" href="' . esc_url( wc_get_cart_url() ) . '" title="' . esc_attr__( 'Shikoni shportën tuaj të blerjes', 'botiga' ) . '">';
...
if ( ! function_exists( 'botiga_woocommerce_header_cart' ) ) {
	...
	function botiga_woocommerce_header_cart() {
		...
		<?php if ( $show_account ) : ?>
		<?php echo '<a class="header-item wc-account-link" href="' . esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ) . '" title="' . esc_html__( 'Llogaria juaj', 'botiga' ) . '"><i class="ws-svg-icon">' . botiga_get_svg_icon( 'icon-user', false ) . '</i></a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		<?php endif; ?>	
		...
		<div id="site-header-cart" class="site-header-cart header-item">
			...
			<?php
			$instance = array(
				'title' => esc_html__( 'Shporta juaj', 'botiga' ),
			);
...
function botiga_sale_badge( $html, $post, $product ) {
	...
	$text 			= get_theme_mod( 'sale_badge_text', esc_html__( 'Zbritje!', 'botiga' ) );
...
function botiga_quick_view_button( $product = false, $echo = true ) {
	...
	<a href="#" class="button botiga-quick-view-show-on-hover botiga-quick-view botiga-quick-view-<?php echo esc_attr( $quick_view_layout ); ?>" aria-label="<?php /* translators: %s: quick view product title */ echo sprintf( esc_attr__( 'Quick view the %s product', 'botiga' ), absint( get_the_title( $product_id ) ) ); ?>" data-product-id="<?php echo absint( $product_id ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'botiga-qview-nonce' ) ); ?>">
		<?php esc_html_e( 'Shihe produktin', 'botiga' ); ?>
	</a>
	<?php
	if( $echo == false ) {
		$output = ob_get_clean();
						...
						<tr>
							...
							<td class="value">
								<?php
									...
									echo end( $attribute_keys ) === $attribute_name ? wp_kses_post( apply_filters( "botiga_{$hook_prefix}_reset_variations_link", '<a class="reset_variations" href="#">' . esc_html__( 'Fshije', 'botiga' ) . '</a>' ) ) : '';
								?>
							</td>
						</tr>
					<?php endforeach; ?>
